...
fn test_fee_split_with_referrer() {
    ...
    t.referral_client
        .register_referral(&referrer, &String::from_str(&t.env, "Referrer"), &None);
...
fn test_reduce_then_rebuild_same_side_opposite_still_rejected() {
    ...
    // Opposite side is also allowed in the two-sided position model.
    t.client.place_bet(&user, &id, &false, &10_0000000_i128);
    let pos = t.client.get_position(&id, &user);
    assert_eq!(pos.gross, 120_0000000);
}
...
fn test_reject_too_many_bets_across_sides() {
    ...
    for i in 0..=20u32 {
        t.client.place_bet(&user, &id, &(i % 2 == 0), &10_0000000_i128);
    }
}
...
fn test_stroop_exact_fee_accounting() {
    ...
    let amount: i128 = 100_000_001; // odd stroop count — exercises floor/ceil
...
fn test_execute_set_config_rejects_wasm_hash_mismatch() {
    ...
    advance_time(&t.env, CONFIG_DELAY_SECS);
    t.env.as_contract(&t.client.address, || {
        let mut pending: PendingConfigChange =
            t.env.storage().instance().get(&DataKey::PendingConfig).unwrap();
        pending.hashes.leaderboard = BytesN::from_array(&t.env, &[1u8; 32]);
        t.env.storage().instance().set(&DataKey::PendingConfig, &pending);
    });
    t.client.execute_set_config(&t.admin);
}
