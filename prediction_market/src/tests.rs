...
fn test_place_bet_rejects_incompatible_referral() {
    let t = setup();
    // Activate the fake referral config, then create a market with enough
    // duration to outlast the config activation timelock (86 400 s).
    ...
    // Create market AFTER config is live so it outlasts the timelock.
    let id = t.client.create_market(
        &t.admin,
        &String::from_str(&t.env, "Will this reject?"),
        &String::from_str(&t.env, "https://x.png"),
        &Category::Crypto,
        &200_000_u64, // 55+ hours
    );
    let user = Address::generate(&t.env);
    fund_user(&t, &user, 200_0000000);
...
fn last_event_name(env: &Env) -> Symbol {
    ...
    let raw = all.events();
    let last = raw.last().expect("no events emitted");
    // Extract the first topic from the XDR event body.
    let soroban_sdk::xdr::ContractEventBody::V0(ref body) = last.body;
    let topic0 = soroban_sdk::Val::try_from_val(env, &body.topics[0]).unwrap();
